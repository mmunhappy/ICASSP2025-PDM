...
suffix = dataset
suffix = suffix + '_pdm_p{}_n{}_lr_{}_seed_{}'.format(args.num_pos, args.batch_size, args.lr, args.seed)
...
if args.optim == 'sgd':
    ignored_params = list(map(id, net.bottleneck.parameters())) \
                     ...
                     + list(map(id, net.bottleneck_s.parameters())) \
                     + list(map(id, net.classifier_s.parameters()))
    ...
    optimizer = optim.SGD([
        ...
        {'params': net.bottleneck_s.parameters(), 'lr': args.lr},
        {'params': net.classifier_s.parameters(), 'lr': args.lr}],
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        feat1, out1, loss_ort, feats, loss_ch, outs = net(input1, input2)
        ...
        loss_tri = criterion_tri(feat1, labels) + criterion_tri(feats, labels)
        ...
        loss = loss_id + loss_tri + loss_cpm * 0.5 + loss_ort + loss_ch * 0.5 + loss_dcs  # 原始损失
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                ...
                epoch, batch_idx, len(trainloader), loss_ch, loss_dcs,
...
def test(epoch):
    ...
    start = time.time()
    ptr = 0
    # gall_feat2 = np.zeros((ngall, pool_dim // 9))
    # gall_feat4 = np.zeros((ngall, pool_dim // 9))
    # gall_feat6 = np.zeros((ngall, pool_dim // 9))
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            # feat1, feat_att1 = net(input1, input1, test_mode[0])
            # feat = (feat + feat1) / 2
            # feat_att = (feat_att + feat_att1) / 2
    ...
    start = time.time()
    ptr = 0
    # query_feat2 = np.zeros((nquery, pool_dim // 9))
    # query_feat4 = np.zeros((nquery, pool_dim // 9))
    # query_feat6 = np.zeros((nquery, pool_dim // 9))
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            # feat1, feat_att1 = net(input1, input1, test_mode[1])
            # feat = (feat + feat1) / 2
            # feat_att = (feat_att + feat_att1) / 2
    ...
    distmat7 = distmat1 + distmat2 + distmat3 + distmat4 + distmat5 + distmat6
    # distmat7 = (distmat2 + distmat4 + distmat6) * 0.9 + 0.1 * (distmat1 + distmat3 + distmat5)
    ...
    writer.add_scalar('rank1', cmc7[0], epoch)
    writer.add_scalar('mAP', mAP7, epoch)
    writer.add_scalar('mINP', mINP7, epoch)
