...
class MFGM(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.SC1 = CBAMBlock(channel // 4)
        self.SC2 = CBAMBlock(channel // 4)
...
class SNL(nn.Module):
    ...
    def forward(self, x_v, x_n):
        B = x_v.size(0)
        g_x = self.g(x_n).view(B, self.low_dim, -1)

        theta_x = self.theta(x_v).view(B, self.low_dim, -1)
        phi_x = self.phi(x_n).view(B, self.low_dim, -1).permute(0, 2, 1)

        energy = torch.matmul(theta_x, phi_x)
        attention = energy / energy.size(-1)

        y = torch.matmul(attention, g_x)
        y = y.view(B, self.low_dim, *x_n.size()[2:])
        W_y = self.W(y)
        z = W_y + x_v
...
class embed_net(nn.Module):
    def __init__(self, class_num, dataset, arch='resnet50', part_num=6):
        ...
        if self.dataset == 'regdb':  # For regdb dataset, we remove the MFA3 block and layer4.
            ...
            self.PLM = PLM(part_num=self.part_num, dim=pool_dim)
    ...
    def forward(self, x1, x2, modal=0):
        ...
        feats = torch.cat((part_feat, x_pool), dim=1)  # 6 * 2048 + 2048 = 14336  全局特征与局部特征之间进行拼接，
        ...
        if self.training:
            ...
            for i in range(self.part_num):
                for j in range(i + 1, self.part_num):
                    loss_ch += ((((sim[:, i] - sim[:, j]) ** 2).sum(dim=1) / (24 * 9)) ** 0.5).sum()
            loss_ch = - loss_ch / (xp.size(0) * self.part_num * (self.part_num - 1) / 2)
            loss_ch *= 0.5

            return x_pool, self.classifier(feat), loss_ort, feats, loss_ch, self.classifier_s(self.bottleneck_s(feats))
        else:
            return self.l2norm(feat), self.l2norm(self.bottleneck_s(feats))


if __name__ == "__main__":
    x1 = torch.randn(8, 3, 384, 144).cuda()
    x2 = torch.randn(8, 3, 384, 144).cuda()
    net = embed_net(395, 'sysu')
    net.cuda()
    x_pool, out0, loss_ort, feats, loss_dp = net(x1, x2)
    pass
